...
### 2.1 自动配置环境变量的情况
各 `linux` 发行版使用各自包管理器安装的软件、以及 `windows` 使用 `.msi` 或 `xx_setup.exe` 安装的软件，通常已经默认配置好环境变量，无须额外配置。
...
依次输入以上每行 `shell` 指令，若无版本号的输出结果，则代表着 `未正确设置环境变量` 。

### 2.3 手动设置环境变量

1. 首先确认软件安装的位置。通常向环境变量中添加该软件文件夹内的 `bin` 文件夹。对于只有单个可执行程序的软件，则只需要向环境变量中添加该软件文件夹的位置。

2. 不同 `shell` 环境对于环境变量的设置不同：
...
在文件尾部添加：
```
export PATH="$HOME/.local/bin:/path/to/app:$PATH"
```
将其中"path/to/app"更改为 `对应软件的文件夹地址`
随后在终端中：
```bash
source ~/.bashrc
```
...
在终端中输入：
```fish
fish_add_path ~/.local/bin /path/to/app
```
将其中"path/to/app"更改为 `对应软件的文件夹地址`
即可生效。

- 对于 `windows` 用户：
可以直接使用 `windows` 中的图形化界面 `编辑环境变量` 中添加。
...
4. **Cortex-Debug**
   ...
   用于在 vscode 中提供一套对代码的调试功能，配合 `arm-none-eabi-gdb` 使用。
